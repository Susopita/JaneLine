...
run-all-tests: compile
	...
	@echo "--------------------------------------------------"
	@echo "[TEST 6/7] Programa (6): RVC E3 - Loads/Stores/Branches/Jumps comprimidos"
	@cd sim && $(VVP) ../$(SIM_OUT) +mem_file=tests/prog6_rvc_e3.mem +expected_addr=100 +expected_data=100 +allow_all_writes=1
	@echo "--------------------------------------------------"
	@echo "[TEST 7/7] Programa completo (Todos los anteriores)"
